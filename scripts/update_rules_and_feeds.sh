#!/bin/bash
    ...
    if docker-compose -f "${DOCKER_COMPOSE_FILE}" exec -T axlap-suricata suricata-update --suricata-conf /tmp/suricata.yaml --no-reload; then
        ...
        docker-compose -f "${DOCKER_COMPOSE_FILE}" exec -T axlap-suricata sh -c 'kill -HUP $(cat /var/run/suricata/suricata.pid 2>/dev/null || pidof suricata)' >> "${LOG_FILE}" 2>&1
    ...
    echo "OpenCTI connectors update feeds based on their own schedules." | tee -a "${LOG_FILE}"
    ...
    #   ${OPENCTI_URL_INTERNAL}/graphql --data @- <<EOF | jq -r '.data.connectors.edges[] | select(.node.name=="MISP").node.id')
    #   {"query":"query { connectors { edges { node { id name } } } }"}
    # EOF
    # if [ -n "$MISP_CONNECTOR_ID" ]; then
    #   curl -s -X POST -H "Content-Type: application/json" -H "Authorization: Bearer ${OPENCTI_ADMIN_TOKEN}" \
    #     ${OPENCTI_URL_INTERNAL}/graphql --data @- <<EOF
    #     {"query":"mutation ConnectorEdit(\$id: ID!, \$input: [EditInput]!) { connectorEdit(id: \$id, input: \$input) { id } }", "variables":{"id":"${MISP_CONNECTOR_ID}", "input":{"key":"connector_run_now", "value":"true"}}}
    # EOF
    #   echo "Sent run now signal to MISP connector ID ${MISP_CONNECTOR_ID}" | tee -a "${LOG_FILE}"
    # else
    #   echo "Could not find MISP connector ID to trigger." | tee -a "${LOG_FILE}"
    # fi
